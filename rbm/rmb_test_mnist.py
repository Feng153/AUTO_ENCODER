...
def image_beautifier(names, final_name):
    ...
    width, heights = zip(*(i.size for i in images))
...
def gen_display_images():
    suffix = '_image.jpg'
    for n in range(10):
        prefix = '../images_RBM/digit-wise/' + str(n) + '_'
...
if __name__ == '__main__':
    ...
    for n in range(10):
        ...
        x = x.unsqueeze(0).to(rbm.device)
        hidden_image = []
        gen_image = []
        for k in range(rbm.k):
            _, hk = rbm.sample_h(x)
            _, vk = rbm.sample_v(hk)
            gen_image.append(vk.detach().cpu().numpy())
            hidden_image.append(hk.detach().cpu().numpy())
