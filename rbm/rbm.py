...
class RBM:
    def __init__(self, n_visible, n_hidden, lr=0.001, epochs=5, mode='bernoulli', batch_size=32, k=3, optimizer='adam',
                 gpu=True, savefile=None, early_stopping_patience=5):
        self.mode = mode
    ...
    def adam(self, g, epoch, index):
        ...
        m_hat = self.m[index] / (1 - np.power(self.beta_1, epoch)) + (1 - self.beta_1) * g / (
                1 - np.power(self.beta_1, epoch))
...
if __name__ == '__main__':
    ...
    rbm = RBM(10, 100, epochs=80, mode='bernoulli', lr=0.001, optimizer='adam', gpu=True,
              savefile='../model/save_example.pt',
              early_stopping_patience=50)
    print("Before Training:", rbm.vb)
    rbm.train(dataset)
    print("After Training:", rbm.vb)
    rbm.load_rbm('../model/save_example.pt')
