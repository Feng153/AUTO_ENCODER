...
if __name__ == '__main__':
    files = ['csv/DBN_without_pretraining_classifier.csv',
             'csv/DBN_with_pretraining_and_input_binarization_classifier.csv',
             'csv/DBN_with_pretraining_classifier.csv',
             'csv/RBM_pretrained_classifier.csv',
             'csv/RBM_without_pretraining_classifier.csv']
    for file in files:
        for feature in [['test loss', 'train loss'], ['test acc', 'train acc']]:
            ...
            if name == 'acc':
                plt.ylim([-0.01, 1.01])
            plt.savefig('images/' + file[:-4][3:] + '_' + name + '.jpg')
